...
\date{2026\\[2pt]\small Version 1.16.1}
...
\begin{document}
  ...
  \subsection{The open problem after O21}
    ...
    inserting this value into the historical prescription
    $\bstar = 1/(\dpair + 1/2)$ gives $\bstar \approx 0.126$.
    This numerical agreement is phenomenological: the prescription imports an LPS growth
    equation into the fixed-degree Heisenberg cascade and is not derived by shell
    alignment~\cite{Beau2026sgn}.
  ...
  \subsection{The Born--Infeld admissibility bound}
    ...
    The Cosmochrony framework~\cite{Cosmochrony,Beau2026c} states the capacity bound as the
    explicit axiom [A-cap] and adopts the Born--Infeld effective action as the saturation
    candidate compatible with it.
...
spectral eigenvalue along the BFS trajectory.
  ...
  \subsection{Main theorem}
    ...
    \begin{remark}[Role of $\cchi$]
      ...
      under the capacity axiom [A-cap] it is set by the maximal transport capacity and
      connectivity density of the underlying relational substrate~\cite{Cosmochrony,Beau2026c}.
